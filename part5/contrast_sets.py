...
def branches1(node, out):
    if node.splitOn is not None:         
        for key, child in node.children.items():
            out.append({"attr":node.splitOn, "val":key, "stats":child.stats})
            branches1(child, out)
    return out

def branches(node):
    out = []
    return have(branches1(node, out))
...
def contrasts(branches, better):
    for i, branch1 in enumerate(branches):
        ...
        sort(out, lambda x, y: x.muinc > y.muinc)
        print(str(i)+ " max mu: " + out[0])
        sort(out, lambda x, y: x.ninc < y.ninc)
